...
int main(void)
{
	...
	Monitor_Queue = xQueueCreate( 	mainQUEUE_LENGTH, sizeof( struct taskListNode* ) );
...
static void DDS_Task( void *pvParameters )
{
	...
	while(1)
	{
		...
		if(current_hyper + 1500 < xTaskGetTickCount()){
			...
			struct taskListNode* lists[3] = {activeHead, completedHead, overdueHead}; // Fixed
